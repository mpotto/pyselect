...
for index, exp in enumerate(exp_configs):
    ...
    np.savetxt(
        f"experiments/gregorova/se1/results/rffnet_scaling_precisions{index}.txt",
        best_model.precisions_,
    )
    joblib.dump(
        best_model, f"experiments/gregorova/se1/models/reg_rff_scaling{index}.joblib"
    )
...
plt.tight_layout()
plt.show()
np.savetxt("experiments/gregorova/se1/results/rffnet_scaling.txt", np.array(results))
