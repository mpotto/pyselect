...
def hash_vectorize_and_scale(X_train, X_test, y_train, y_test):

    hv = HashingVectorizer(n_features=2000)
    tfidf_vect = TfidfTransformer()

    X_train_hv = hv.transform(X_train)
    X_test_hv = hv.transform(X_test)

    # X_train = tfidf_vect.fit_transform(X_train_hv)
    # X_test = tfidf_vect.transform(X_test_hv)
...
# Vectorize and scale.
X_train, X_test, y_train, y_test, features = hash_vectorize_and_scale(
